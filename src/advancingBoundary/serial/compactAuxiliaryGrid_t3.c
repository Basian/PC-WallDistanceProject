...
void compactAuxiliaryGrid_t3(struct cell_t3 * auxCells, int numAuxCells, struct cell_t3 * compAuxCells, double * xf, double * yf, int numFaces){
	...
	for (i=0; i<numAuxCells; i++){
		for (j=0; j<numFaces; j++){
			...
			if (xf[j] < auxCells[i].xmax && xf[j] > auxCells[i].xmin && yf[j] < auxCells[i].ymax && yf[j] > auxCells[i].ymin){
				...
				auxCells[i].face_x[index] = xf[j];
				auxCells[i].face_x[index] = yf[j];
				auxCells[i].numFaces++;
	...
	for (i=0; i<numAuxCells; i++){
		if(auxCells[i].numFaces != 0){
			...
			for(k=0; k<auxCells[i].numFaces; k++){
				compAuxCells[j].face_x[k] = auxCells[i].face_x[k];
				compAuxCells[j].face_y[k] = auxCells[i].face_y[k];
			}
			j++;
		}
	}
