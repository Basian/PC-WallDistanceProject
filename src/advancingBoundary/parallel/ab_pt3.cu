...
extern "C" {
#include "computeAuxiliaryGrid_pt3.h"
}
...
__global__
void wd_ab_parallel_pt3_t5(double *cellCenters, double *faceCenters, double *box, struct cell_pt3 *compAuxCells, int size_c, int size_f, int numAuxCells, double auxDiag, double *wallDist){
	...
	for (j=0; j<numAuxCells; j++){
		...
		if(rAux < r){
			...
			while(index < compAuxCells[j].numFaces){
				rtemp = sqrt( pow(c_x-compAuxCells[j].faces[2*index],2) + pow(c_y-compAuxCells[j].faces[2*index+1],2));
...
void ab_parallel_t3(double * xc, double * yc, double * xf, double * yf, int size_c, int size_f, double * wallDist){
	...
	struct cell_pt3 *auxCells;
//	auxCells = (struct cell_pt1 *)malloc(numAuxCells*sizeof(struct cell_pt1));
	auxCells = new cell_pt3[numAuxCells];


	computeAuxiliaryGrid_pt3(xmin,xmax,ymin,ymax,resI,resJ,auxCells);
	...
	struct cell_pt3 * compAuxCells;
//	compAuxCells = (struct cell_pt1 *)malloc(cellsWithFaces*sizeof(struct cell_pt1));
	compAuxCells = new cell_pt3[cellsWithFaces];


	///////
	compactAuxiliaryGrid_pt3(auxCells,numAuxCells,compAuxCells,xf,yf,size_f);
	///////
	...
	// auxCell structs
	struct cell_pt3 * d_compAuxCells;
	checkCudaErrors(cudaMalloc((void **)&d_compAuxCells,cellsWithFaces*sizeof(struct cell_pt3)));
	checkCudaErrors(cudaMemcpy(d_compAuxCells,compAuxCells,cellsWithFaces*sizeof(struct cell_pt3),cudaMemcpyHostToDevice));
...
//	 Reset wallDistance
	...
	timer.Start();
	wd_ab_parallel_pt3_t5<<<numBlocks,threadsPerBlock,16*sizeof(double)>>>(d_cellCenters,d_faceCenters,d_box,d_compAuxCells,size_c,size_f,cellsWithFaces,auxDiag,d_wallDist);
	timer.Stop();
	printf("Advancing boundary - parallel pt3_T5(GpuTimer): \t %.0f milliseconds\n",timer.Elapsed());
